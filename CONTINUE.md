...
- corrected Catholic localized names in JA/EN/KO/PT/ID so the redundant country qualifier is omitted (for example, `Hagi Catholic Church`), and simplified Catholic detail slugs to `<place>-catholic-church.html` without the `catholic-church-in-japan` prefix.
- added a durable Google CID exclusion list and excluded `google:1885579487859421798` at `〒881-0005 宮崎県西都市三宅３３５６`: Google labels it `カトリック教会`, but its category and on-site evidence identify a cemetery rather than a congregation. Removed it from the catalog and Neo4j, rebuilt the 9,464-church snapshot, reached 9,464/9,464 parity, passed all 10 integrity checks, and regenerated 47,320 localized church detail pages.
