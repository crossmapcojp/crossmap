...
The `crawl` module turns raw church-source data and downloaded web evidence into committed revisions in the canonical Neo4j church catalog. It also builds geonames, resolves denominations and social accounts, assigns mandatory English church names, and publishes versioned search-index snapshots.

## Where does the data begin?

There are two inputs to understand:

1. **Frozen migration input:** `resources/catalog/churches.json` was created once from the clean data under the old gmap `output` directory. It is retained only for bootstrap, parity diagnostics, and rollback; normal workflows must never read, edit, or overwrite it. The copied HTML now lives in `cache/web-pages`.
...
flowchart LR
    ...
    F[Current committed Neo4j revision] --> I
    I --> J[New committed Neo4j revision]
```
...
| Official GeoNames alternate names | `cache/geoname/japan/alternatenames/JP.txt` | Language-tagged alternate names (en/ko/pt/id/vi) |
| JMA multilingual city dictionary | [`resources/geonames/jma-city.json`](resources/geonames/jma-city.json) | en/ko/pt/id/vi translations maintained by Japan Meteorological Agency |
| Municipality source | (supplied at build time) | `code → "name"` mapping for all municipalities |
| Church catalog | Current committed Neo4j revision | Church coordinates used to compute geoname centers and radii; loaded through `CrawlCanonicalCatalog` |
...
- `cache/geoname/japan/church-name-multilingual-lexicon.json` — Japanese-to-{en,ko,pt,id,vi} map from official alternate names + JMA city dictionary.
...
| `google-saved-places` | [`GoogleSavedPlacesCrawler.kt`](src/main/kotlin/jp/co/crossmap/crawl/GoogleSavedPlacesCrawler.kt) | Run the complete Google Saved Places workflow based on Takeout CSV files (CSV reading, CID resolution, and catalog promotion). | `crossmap.googleSavedPlaces` path in `local.properties`; Saved-list CSV files; current committed Neo4j revision | new committed Neo4j revision and pipeline logs | `logs/YYYY-MM-DD-HH-mm-*.log` |
| `refresh` | [`ChurchWebsiteCrawler.kt`](src/main/kotlin/jp/co/crossmap/crawl/ChurchWebsiteCrawler.kt) | Breadth-first same-domain crawl with bounded `--max-depth 0..3` and page count; records outgoing links and extracts official localized names from locale home-page headings. Listing aggregators and Google Maps fallbacks are never fetched. | catalog, exclusion catalog, prior crawl manifest/cache | `cache/web-pages`, manifest, URL map, depth/language/outgoing-link evidence, official localized names | `logs/YYYY-MM-DD-HH-mm-refresh.log` |
| `crawl-denomination-directories` | [`denomination/OfficialDenominationChurchListPipeline.kt`](src/main/kotlin/jp/co/crossmap/crawl/denomination/OfficialDenominationChurchListPipeline.kt), [`OfficialDirectoryCrawler.kt`](src/main/kotlin/jp/co/crossmap/crawl/OfficialDirectoryCrawler.kt) | Parse authoritative UCCJ/JBC tables and the JBBF address book, replace their stale candidates, reconcile the catalog one-to-one, then crawl other configured directories unless `--dedicated-only` is set. `--force-refresh` invalidates and refetches all three authoritative pages. | official UCCJ/JBC/JBBF pages, `sources/denominations.json`, exclusion catalog, current committed Neo4j revision | `crawl/uccj-churches.json`, `crawl/jbc-churches.json`, `crawl/jbbf-churches.json`, `cleanup/denomination-candidates.json`, new committed Neo4j revision | `logs/YYYY-MM-DD-HH-mm-crawl-denomination-directories.log` |
...
| `validate-chinese-dictionaries` | [`ChineseDictionaryValidator.kt`](src/main/kotlin/jp/co/crossmap/crawl/ChineseDictionaryValidator.kt) | Validate paired Chinese dictionaries and emit nonfatal terminology review signals. | `resources/dictionary/ja-zh-{Hans,Hant}-*.csv` | `resources/review/chinese-dictionary-validation.json` | `logs/YYYY-MM-DD-HH-mm-chinese-dictionary-validation.log` |
| `localize-chinese-names` | [`ChineseLocalizationMigration.kt`](src/main/kotlin/jp/co/crossmap/crawl/ChineseLocalizationMigration.kt) | Idempotently add zh-Hans/zh-Hant church and minister records, preserve official/manual/reviewed values, and produce confidence/coverage review reports. Use `--dry-run` before applying. | canonical church catalog and localization dictionaries | optional updated catalog plus JSON/text reports under `resources/review` | `logs/YYYY-MM-DD-HH-mm-chinese-localization.log` |
...
| `localize-vietnamese-names` | [`VietnameseLocalizationMigration.kt`](src/main/kotlin/jp/co/crossmap/crawl/VietnameseLocalizationMigration.kt) | Idempotently add Vietnamese church and minister records, preserve official/manual/reviewed values, and produce confidence/coverage review reports. Use `--dry-run` before applying. | canonical catalog plus JA→VI, congregation, denomination, and GeoName dictionaries | optional updated catalog plus Vietnamese JSON/text reports under `resources/review` | `logs/YYYY-MM-DD-HH-mm-vietnamese-localization.log` |
| `build-snapshot` | [`SnapshotBuilder.kt`](src/main/kotlin/jp/co/crossmap/crawl/SnapshotBuilder.kt) | Build/package JA/EN/KO/PT/ID/VI/zh-Hans/zh-Hant Lucene indexes; Vietnamese uses `VietnameseAnalyzer`, while Chinese indexes preserve script-specific exact/analyzed fields and canonical Simplified search fields. | complete catalog, geonames, translation catalog/usage, optional normalized-address cache | `cache/search-indexes/churches/VERSION/index/{ja,en,ko,pt,id,vi,zh-Hans,zh-Hant}` and latest metadata | `logs/YYYY-MM-DD-HH-mm-build-snapshot.log` |
| `catalog-neo4j-status` | [`CatalogNeo4jCommands.kt`](src/main/kotlin/jp/co/crossmap/crawl/CatalogNeo4jCommands.kt) | Check Neo4j reachability, schema health, and the current committed revision/hash. | Neo4j connection via `local.properties` or environment variables | console status report | console output |
...
| `catalog-neo4j-bootstrap-from-legacy-json` | [`CatalogNeo4jCommands.kt`](src/main/kotlin/jp/co/crossmap/crawl/CatalogNeo4jCommands.kt) | One-time bootstrap of a legacy `churches.json` catalog into Neo4j. This is not a normal pipeline command. `--dry-run` validates without writing. | Neo4j connection, `--input` legacy churches.json path | import report under `build/reports/catalog-import` | console output |
| `catalog-neo4j-export-church-projection` | [`CatalogNeo4jCommands.kt`](src/main/kotlin/jp/co/crossmap/crawl/CatalogNeo4jCommands.kt) | Generate a diagnostic/human-readable church projection plus a revision and file-hash manifest. It is not canonical. | Neo4j connection | `build/reports/catalog-export/churches.json` and neighboring manifest | console output |
...
- `./gradlew :crawl:prepareChurchGeoNames` first rebuilds `japan.json` from JMA data (including designated-city wards and canonical vi/zh-Hans/zh-Hant translation records), then refreshes the reviewable title/address translation catalog and per-language missing CSVs. `./gradlew :crawl:normalizeChurchAddresses` depends on it and reads `crossmap.geoloniaNormalizerDir` from `local.properties`. `:crawl:buildSearchSnapshot` depends on all stages before creating the eight language indexes.
- Chinese operations are `:crawl:validateChineseDictionaries`, `:crawl:dryRunChineseLocalizedNames`, `:crawl:produceChineseReviewReport`, `:crawl:generateChineseLocalizedNames`, `:crawl:chineseGoldenTest`, and `:crawl:reindexChineseFields`. Verbose rule matches, unmatched segments, and review explanations live in `resources/review/chinese-localization-report.json`; generated catalog records retain only compact provenance, and the catalog writer omits formatting/default-valued fields. See [`docs/development/chinese-localization.md`](../docs/development/chinese-localization.md).
...
- `GoogleMapsPlaceParser` extracts the Google place title, coordinates, address, website, and category, immediately applies [`ChurchWebsitePolicy`](../core/src/commonMain/kotlin/jp/co/crossmap/ChurchWebsitePolicy.kt), then invokes the authoritative multilingual name workflow. When a bilingual Saved Places title contains a promotional Japanese alias, the fetched canonical Japanese title wins while a church-shaped Latin title has a trailing ` - ...` promotion removed. An excluded or missing website becomes `https://www.google.com/maps?cid=<googleCid>`.
- `ChurchWebsiteCrawler` follows prioritized same-domain hyperlinks breadth-first. Depth defaults to 1 and is configurable through 3; each fetched `CrawledPage` stores depth, language, and outgoing URLs. `ChurchWebsiteOfficialNameExtractor` trusts a church-shaped primary heading and locale path ahead of faulty HTML language metadata, so `tokyochurch.org`, `/j/`, and `/c/` yield the exact English, Japanese, and Simplified-Chinese TMC names. Manual and reviewed names remain protected. Neo4j import uses normalized URL IDs and idempotent `LINKS_TO` edges, including discovered-only target pages.
...
- `MultilingualChurchNameLocalizer` performs all title-name work together: decomposition, per-part source language and role detection, deterministic phrase translation, and ordered `localizedNames` composition. It preserves the original-language title and emits exactly one nonblank JA/EN/KO/PT/ID/VI/zh-Hans/zh-Hant name per language. Vietnamese and Chinese outputs carry provenance, confidence, matched/unmatched segments, and review status; unmatched or Kana-retaining output is reported instead of failing ingestion. Internal sentinel and Romance-language ordering behavior remains unchanged.
...
- `EnglishJapaneseAddressMatcher` scores official romanized addresses against Japanese catalog addresses using conflicting/exact postal codes, street-number sequences, and prefecture evidence. Official-list reconciliation uses this score alongside the existing Japanese normalized-address score; this enables directories such as JCOB without weakening exact Japanese matching.
