...
| `crawl`, `core`, `resources` | `./gradlew :crawl:googleSavedPlaces` | Runs the entire Google Saved Places workflow based on the downloaded Takeout CSV files (CSV reading, CID resolution, and catalog promotion). | Reads Takeout CSV path from `local.properties`; updates canonical catalog after all quality gates pass. | `resources/catalog/churches.json` and stage logs. |
