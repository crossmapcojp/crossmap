...
## Pre-migration Crossmap architecture snapshot

- Repository: the Crossmap project root.
...
- At the time of this assessment, the crawl pipeline generated and mutated `resources/catalog/churches.json`. That legacy architecture has since been replaced by the canonical revision model documented in [`catalog-json-migration.md`](catalog-json-migration.md); this section is historical evidence, not current operating guidance.
...
- A local Community instance was available during the assessment; current operation uses the repository-local instance described in [`neo4j-local.md`](neo4j-local.md).
